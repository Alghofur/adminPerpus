<?php 
    ...
    //menampilkan data dari tabel anggota
    global $conn;
    ...
    if(isset($_POST['ubah'])){
      ...
      if(empty($nim && $nama && $tempat_lahir && $tgl_lahir && $jk && $prodi)){
        echo "<script>alert('Pastikan anda mengisi semua datanya'; document.location.href = 'anggota.php'</script>";
      }

      $sql = $conn->query("UPDATE anggota SET nim = '$nim', nama_anggota = '$nama', tempat_lahir = '$tempat_lahir', tgl_lahir = '$tgl_lahir', jk = '$jk', prodi = '$prodi' WHERE id_anggota = $id_anggota") or die(mysqli_error($conn));
      if($sql){
        echo "<script>alert('Data Berhasil Diubah.'); document.location.href = 'anggota.php';</script>";
      }else{
        echo "<script>alert('Data Berhasil Diubah.'); document.location.href = 'ubah.php';</script>";
      }
    }
?>
<!DOCTYPE html>
