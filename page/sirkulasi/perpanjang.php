...
if($lambat > 3) {
	...
	$sql = $conn->query("UPDATE transaksi SET tgl_kembali = '$hari_next' WHERE id_transaksi = $id_transaksi") or die(mysqli_error($conn));

	if($sql) {
		echo "<script>alert('Perpanjang jangka waktu buku berhasil.');window.location.href='sirkulasi.phpz'</script>";
	} else {
		echo "<script>alert('Perpanjang gagal.');</script>";
	}
}
